...
You can edit both objects freely in Blender before exporting; the only contract is that the file contains an object with `l` lines and no faces (the grid) and an object with `f` lines and no `l` lines (the dual), in the same coordinate system. An object with both faces and edges is rejected with an explanation, since either reading of it would be silently wrong; Blender's `o` objects and `g` groups are both accepted as object boundaries. Keep both objects visible when exporting: Blender's exporter skips hidden objects, so *Export for mars* unhides them for the export and refuses if it cannot select them. The dual may also come in a separate file (`-d dual.obj`). `mars-cli` links every dual face to the grid edge it bisects by geometry: the two grid points nearest to the face's centroid are its edge, and all its vertices must be equidistant from them. Faces that fail this test are the outer walls of the boundary cells and are dropped; faces that appear twice are merged; a triangulated dual is rejected. The grid's edge set is the union of the `l` lines and the matched faces, so the `l` lines are optional when a dual is present. Check the result with `mars-cli grid-check`.
